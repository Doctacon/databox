...
export function publicCatalogPhoto(
  ...
  if (
    media.kind !== "photo" || media.provider !== "usfws"
    ...
    || typeof media.source_url !== "string" || !USFWS_SOURCE_URL.test(media.source_url)
    ...
    || typeof media.license !== "string" || PHOTO_LICENSES.get(media.license) !== media.license_url
  ) return null;
  return {
    status: "available",
