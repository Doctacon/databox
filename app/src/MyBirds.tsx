...
export function MyBirdsPage({ navigate }: { navigate: Navigate }) {
  ...
  useEffect(() => {
    ...
    void Promise.all([listBirds(), listObservations(), listLifeList(), listWishlist(), listWatches()]).then(([catalog, obs, lifeRows, wishRows, watchRows]) => {
      ...
      setBirds(catalog); setObservations(obs); setLife(lifeRows); setWishlist(wishRows); setWatches(watchRows); setNewWatchSpecies((value) => value || catalog[0]?.species_code || ""); setCollectionLoaded(true);
