...
export function MyBirdsPage({ navigate }: { navigate: Navigate }) {
  ...
  useEffect(() => {
    ...
    void Promise.all([listBirds(), listObservations(), listLifeList(), listWatches()]).then(([catalog, obs, lifeRows, watchRows]) => {
      ...
      setBirds(catalog); setObservations(obs); setLife(lifeRows); setWatches(watchRows); setNewWatchSpecies((value) => value || catalog[0]?.species_code || ""); setCollectionLoaded(true);
