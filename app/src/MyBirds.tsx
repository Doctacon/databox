...
const COLLECTION_TABS: Tab[] = PUBLIC_RUNTIME
  ? ["life", "observations", "watches"]
  : ["life", "observations", "watches", "alerts"];

function watchEvaluationText(value: BrowserWatchEvaluation | "loading" | "unavailable" | undefined): string {
  if (!value || value === "loading") return "Checking the published snapshot…";
  if (value === "unavailable") return "Published watch matching is temporarily unavailable. Your saved watch is still on this device.";
  if (value.match_count === 0) return "No licensed historical occurrences in the published snapshot fall inside this watch radius.";
  const newest = value.latest_observation_at ? new Date(value.latest_observation_at).toLocaleDateString() : "unavailable";
  return `${value.match_count.toLocaleString()} licensed historical match${value.match_count === 1 ? "" : "es"} inside this radius · nearest ${value.nearest_location_name || "published location"} (${value.nearest_distance_miles?.toFixed(1) ?? "—"} mi) · newest ${newest}`;
}
...
function WatchForm({ speciesCode, initial = null, busy, onCancel, onSave }: WatchFormProps) {
  const initialCenter: LocationSelection | null = initial ? {
    ...
    timezone: initial.center_timezone as "America/Phoenix" | "America/Denver", region_code: "US-AZ",
  } : null;
...
export function MyBirdsPage({ navigate }: { navigate: Navigate }) {
  ...
  const [watchEvaluations, setWatchEvaluations] = useState<Record<string, BrowserWatchEvaluation | "loading" | "unavailable">>({});
  ...
  useEffect(() => {
    if (!PUBLIC_RUNTIME || !collectionLoaded) return;
    let current = true;
    const active = watches.filter((watch) => watch.active);
    setWatchEvaluations(Object.fromEntries(active.map((watch) => [watch.species_code, "loading"])));
    if (!active.length) return () => { current = false; };
    void import("./publicAdapters/watchEvaluation").then(({ evaluateBrowserWatch }) => Promise.all(active.map(async (watch) => {
      try { return [watch.species_code, await evaluateBrowserWatch(watch)] as const; }
      catch { return [watch.species_code, "unavailable" as const] as const; }
    }))).then((rows) => { if (current) setWatchEvaluations(Object.fromEntries(rows)); })
      .catch(() => {
        if (current) setWatchEvaluations(Object.fromEntries(active.map((watch) => [watch.species_code, "unavailable"])));
      });
    return () => { current = false; };
  }, [collectionLoaded, watches]);
  ...
  return <main className="my-birds-main">
    <header className="catalog-heading"><p className="eyebrow">Private local collection</p><h1 ref={headingRef} tabIndex={-1}>My Birds</h1><p>{PUBLIC_RUNTIME
      ? "Observations, life list, and watched birds stay only in this browser until you remove them."
      : "Observations, life list, and watched birds remain in your local DuckDB until you remove them."}</p></header>
    <nav className="collection-tabs" aria-label="My Birds sections">{COLLECTION_TABS.map((value) => <button key={value} type="button" aria-pressed={tab === value} onClick={() => setTab(value)}>{value === "life" ? "Life List" : value === "alerts" ? "Alert Delivery" : value[0].toUpperCase() + value.slice(1)}</button>)}</nav>
    ...
    {deletingWatch && <ConfirmDialog title="Delete this watch?" busy={busy} confirmLabel="Delete watch" onCancel={() => setDeletingWatch(null)} onConfirm={() => void mutate(() => deleteWatch(deletingWatch.species_code), "Watch deleted.").then((saved) => { if (saved) setDeletingWatch(null); })}>{PUBLIC_RUNTIME
      ? "The watch is removed from this browser. Rufous has not scheduled an email or background delivery."
      : "The watch definition is removed. A downstream alert worker may later cancel an accepted active calendar event."}</ConfirmDialog>}
  </main>;
}
