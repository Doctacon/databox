...
export function FieldMapPage({ navigate }: { navigate: Navigate }) {
  ...
  return <main className="field-map-main">
    ...
    {snapshot && <>
      ...
      <span className="map-result-count sr-only" aria-live="polite">{filtered.length.toLocaleString()} {PUBLIC_RUNTIME ? `licensed historical occurrence${filtered.length === 1 ? "" : "s"}` : `eligible encounter${filtered.length === 1 ? "" : "s"}`}</span>
