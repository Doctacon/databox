...
function Result({ plan }: { plan: TargetPlan }) {
  ...
  return <section className="target-result" aria-labelledby="target-result-heading">
    ...
    <p className="source-status">{PUBLIC_RUNTIME ? "Saved in this browser" : "Saved local target plan"} · {plan.taxonomic_category} · {plan.species_code}</p>
    ...
    <section className="panel"><h2>Qualifying public locations</h2>
      {plan.candidates.length ? <ol className="location-list">{plan.candidates.map((candidate) => <li key={candidate.location_id}>
        ...
        <span>{candidate.observation_count.toLocaleString()} {PUBLIC_RUNTIME ? "licensed occurrence record" : "independent submission"}{candidate.observation_count === 1 ? "" : "s"} · newest {date(candidate.latest_observation_at)}</span>
    ...
    <section className="panel"><h2>Grounded guidance</h2><ol>{plan.guidance.map((item) => <li key={item}>{item}</li>)}</ol>
      <p className="source-status">{PUBLIC_RUNTIME
        ? "Generated deterministically in your browser from the published target, candidate, radius, and window facts. No AI model was called."
        : "Generated by the sole configured Cloudflare GLM 4.7 Flash model from the exact persisted target, candidate, radius, window, and weather facts."}</p>
    </section>
...
export function TargetBirdPage({ speciesCode, planId, navigate }: { speciesCode?: string; planId?: string; navigate: Navigate }) {
  ...
  return <main className="bird-profile-main">
    ...
    {plan ? <Result plan={plan} /> : bird ? <>
      <header className="hero-card"><p className="eyebrow">Target-bird planner · {bird.taxonomic_category}</p><h1 ref={formHeadingRef} tabIndex={-1}>Find {bird.common_name || bird.scientific_name || bird.species_code}</h1><p>{PUBLIC_RUNTIME
        ? "Use licensed, generalized historical occurrence evidence inside your chosen travel radius."
        : "Use current valid, reviewed, non-private eBird evidence inside your chosen travel radius."}</p></header>
