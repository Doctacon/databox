...
- 2026-09-03: Reopened after confirming the repository has `DATABOX_AWS_ROLE_ARN` rather than static AWS keys. Updated the manual workflow to use GitHub OIDC, short-lived role credentials, and job-generated disposable Polaris/Postgres values. The existing `DATABOX_AWS_S3_BUCKET` secret supplies the bucket; provider tokens remain secret references. OIDC session credentials including the session token are mapped into the existing compose contract. Independent review and hosted PR #41 CI passed.

## Blockers

None. The manual real-integration workflow awaits explicit dispatch after merge or a deliberately authorized branch dispatch.
