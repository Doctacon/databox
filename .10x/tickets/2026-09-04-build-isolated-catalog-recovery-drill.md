...
- 2026-09-09: User authorized one exact live read-only run. Evidence `.10x/evidence/2026-09-09-restored-catalog-live-validation.md` records exit `1` after 54.956 seconds: all 22 registry-derived tables were present and passed metadata/current-snapshot/manifest/limit-one reads with zero table failures, but fail-closed inventory comparison found seven unexpected tables. Three are generated eBird taxonomy children, three belong to explicitly excluded USFWS, and one belongs to a probe namespace. Active and recovery services remained unchanged, healthy/running, and unexposed. Drift ownership moved to `.10x/tickets/2026-09-09-reconcile-restored-catalog-registry-drift.md`; no retry or mutation occurred.

## Blockers

Resolve and independently review `.10x/tickets/2026-09-09-reconcile-restored-catalog-registry-drift.md`, then separately authorize a new live validation. Do not delete or reuse recovery artifacts without authorization. Complete failure-path coverage and the timed drill remain unimplemented.
