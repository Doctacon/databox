...
Review and approve installing Debian `ca-certificates` in the PostgreSQL image; never disable TLS verification. Then rebuild and rerun the authorized proof. Before recreating Polaris through Compose, resolve its required primary `DATABOX_AWS_SESSION_TOKEN` without persisting or fabricating a session token. Polaris must remain unavailable until stanza, WAL, and required backup validation pass.
