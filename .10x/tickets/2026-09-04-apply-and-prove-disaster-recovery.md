...
Generate a new normal-refresh plan from the actual `infra/recovery` directory with its current lock file and state; verify lineage/dependency compatibility; independently review and explicitly approve the new hash. After a successful apply, manually enable console access and MFA before role-assumption proof. Preserve `infra/recovery/terraform.tfstate` and include it in the operator's encrypted machine backup.
