...
Repair the three significant gate findings in `.10x/reviews/2026-09-04-fail-closed-backup-gate-review.md`. Repository automation also still needs scheduling, repository verification, encrypted logical export, and deterministic catalog inventory. Live repository checks require provisioned infrastructure and belong to the live apply/proof ticket. The real image build, Compose render, pgBackRest/WAL round trip, and initial backup remain unproven in this no-live slice.
