...
The three findings in `.10x/reviews/2026-09-04-fail-closed-backup-gate-review.md` are resolved by credential injection, corrected initial-backup sequencing, and explicit supersession of ongoing enforcement. Repository automation still needs startup cadence and post-backup metadata verification, encrypted logical export, and deterministic catalog inventory. Live repository checks require provisioned infrastructure and belong to the live apply/proof ticket. The real image build and pgBackRest/WAL/initial-backup round trip remain unproven in this no-live slice.
