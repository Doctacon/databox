...
Root was used only for the explicitly approved bootstrap apply, but live verification proved AWS root cannot call `sts:AssumeRole`; the planned runtime flow is therefore blocked. OpenTofu MUST declare the non-root `databox-recovery-operator` IAM user without an access key, login profile, password, or MFA material; grant it `sts:AssumeRole` only on `databox-polaris-catalog-backup` plus exactly `signin:AuthorizeOAuth2Access` and `signin:CreateOAuth2Token` only on `arn:aws:signin:us-west-1:734815189723:oauth2/public-client/remote`; and make that role trust the exact user ARN only when `aws:MultiFactorAuthPresent=true`. This supersedes the earlier assume-only policy because live proof showed AWS rejects `aws login --remote` without the two sign-in actions. Console access, password setup, and MFA enrollment are a manual second step and MUST never enter configuration or state. The human supplies MFA during `AssumeRole`; AWS returns ordinary short-lived role credentials for host injection, and pgBackRest never receives MFA data. Runtime pgBackRest credentials MUST come from the least-privilege role and MUST never be root credentials. Role assumption MUST succeed before root is logged out and backup operation begins.
