...
The initial architecture selected `@cf/zai-org/glm-4.7-flash`, then temporarily replaced it with `@cf/zai-org/glm-5.2` after bounded live requests timed out. Cloudflare now requires Workers Paid for GLM 5.2, while this installation uses Workers AI Free. The user therefore explicitly restored `@cf/zai-org/glm-4.7-flash`, using `CF_WORKERS_AI_MODEL_BASE_URL='@cf/zai-org/glm-4.7-flash'` and retaining the hard allowlist and no-fallback rule. A bounded live request now validates the free-tier runtime contract.
...
7. The only allowed model identifier is `@cf/zai-org/glm-4.7-flash`. No fallback or alternate model is permitted.
8. The local model selector is `CF_WORKERS_AI_MODEL_BASE_URL='@cf/zai-org/glm-4.7-flash'`; the client derives Cloudflare's fixed HTTPS endpoint and never sends credentials to another host.
...
- **Keep `@cf/zai-org/glm-5.2`**: rejected because it requires Workers Paid and cannot satisfy this installation's free-tier runtime contract.
...
- Historical evidence for the earlier GLM 4.7 Flash and GLM 5.2 periods remains historical and must not be rewritten as proof for the current runtime.
- Parent closure requires a successful bounded live GLM 4.7 Flash inference or a newly ratified acceptance change.
