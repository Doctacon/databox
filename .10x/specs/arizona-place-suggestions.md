...
4. Fetch bounded Open-Meteo results only when fewer than the response limit are available locally. Local hotspot results precede city fallback. Deduplicate exact source IDs and near-identical normalized label/coordinates.

Default/max response limit remains bounded by the existing contract. Upstream failure returns local results when any exist; only zero-local plus failed fallback yields safe geocoder-unavailable behavior. Valid manual Arizona coordinates remain supported.
...
- `Prescott` can return local hotspots followed by the Prescott city result.
