...
- The interactive drill command MUST fail before mutation when authentication, TTY, repository, active-state, or resource-name preconditions fail; orchestrate marker-bracketed WAL, new-volume restore, isolated PostgreSQL/Polaris startup, registry-derived validation, and RPO/RTO timing; clean its active marker; and preserve isolated recovery artifacts for separately authorized cleanup.
