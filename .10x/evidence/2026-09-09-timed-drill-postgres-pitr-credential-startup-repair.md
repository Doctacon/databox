...
Focused recovery and validator tests passed, including exact phase ordering, backup names only on the first PostgreSQL exec, no names on the second exec, promotion validation before restart, post-scrub validation before Polaris, PostgreSQL quiescence on every exception or RPO/RTO objective failure after its start attempt, and credential-free PostgreSQL remaining available only after full success. Objective-failure tests preserve measured results and nonzero CLI behavior. Disposable no-network pinned-image probing used dummy values only and no mounts; no AWS, active SQL mutation, marker cleanup, retry, artifact deletion, or cutover occurred during the repair.
