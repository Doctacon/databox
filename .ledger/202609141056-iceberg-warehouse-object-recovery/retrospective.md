...
The later Stage 2C campaign exposed two additional integration seams. Restarting PostgreSQL into archival mode invalidates Polaris's JDBC pool, so Polaris and its cached gateway must be recreated after the point-A backup. Docker `--network none` is not represented as an empty runtime network map; it is a built-in `none` network whose endpoint has no routable address. Fail-closed ownership checks should be derived from actual runtime representations and tested before a destructive boundary. The three-attempt campaign still stopped safely: its sole damage cycle passed break proof, exact objects were restored, the exact catalog target was restored, and every generated container was removed. But restoration after a terminal failure is not proof; because final joint Polaris validation did not run, Stage 2C remains incomplete.
