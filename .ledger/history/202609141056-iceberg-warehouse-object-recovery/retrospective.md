...
Docker ownership validation must distinguish configured publishing intent from runtime assignment. For an ephemeral loopback publish, `HostConfig.PortBindings` may retain an empty host-port string while `NetworkSettings.Ports` contains the assigned numeric port; image-exposed but unpublished ports appear there with null values. Stage 2C's first live campaign stopped safely before point A because it treated those equivalent views as one field. The correction validates both views independently and still rejects non-loopback publication.

The later Stage 2C campaign exposed two additional integration seams. Restarting PostgreSQL into archival mode invalidates Polaris's JDBC pool, so Polaris and its cached gateway must be recreated after the point-A backup. Docker `--network none` is not represented as an empty runtime network map; it is a built-in `none` network whose endpoint has no routable address. Fail-closed ownership checks should be derived from actual runtime representations and tested before a destructive boundary. The three-attempt campaign still stopped safely: its sole damage cycle passed break proof, exact objects were restored, the exact catalog target was restored, and every generated container was removed.

The retained-state continuation finally proved the missing two-table Polaris equality, but then exposed a third distinction: an S3 copy promotion creates a new version whose ETag need not equal the historical source version's ETag even when SHA-256 and size are exact. Source ETags remain valid for conditional source selection and source-version identity; they are not cross-version content digests. Final inventory must bind source and promoted VersionIds separately and compare exact current bytes. A post-validation harness failure can be adjudicated without replay only when control-flow phase evidence and independent read-only diagnosis prove the substantive assertions already passed; the command and RTO must still remain failures.

Terminal cleanup reinforced that ownership identity and runtime attachment state should be checked separately. Stopping an attached container can change Docker network inspection fields that are irrelevant to network ownership. The initial cleanup correctly failed closed after completing S3 deletion and container containment; a fresh plan, rather than replay, then bound the new state and completed removal using immutable network configuration plus immediate attachment checks. Final verification preserved every pre-cleanup S3 timeline entry and the accepted private evidence while removing all live synthetic objects and Docker remnants.
