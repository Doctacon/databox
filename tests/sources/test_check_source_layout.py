"""Executable contract tests for scripts/sources/check_source_layout.py."""
...
SCRIPT = Path(__file__).parents[2] / "scripts" / "sources" / "check_source_layout.py"
...
def _entry(
    ...
    *,
    ...
    return Source(
        ...
        orchestration_mode=cast(Any, orchestration_mode),
    )
...
def test_explicit_target_source_passes_without_unconfigured_asset_or_job(
    tmp_path: Path,
) -> None:
    module = _load_module()
    _rebind(module, tmp_path)
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            "must omit unconfigured domain asset",
        ),
        ("assets = []", "assets = [object()]", "assets must be assigned an empty list"),
        (
            "dlt_asset_keys = []",
            "dlt_asset_keys = [object()]",
            "dlt_asset_keys must be assigned an empty list",
        ),
        (
            "ingest_job = None",
            "ingest_job = dg.define_asset_job(name='unsafe')",
            "ingest_job must be assigned None",
        ),
    ],
)
def test_explicit_target_source_rejects_unsafe_default_exports(
    tmp_path: Path, valid: str, unsafe: str, message: str
) -> None:
    module = _load_module()
    _rebind(module, tmp_path)
    _source(
    ...
    domain.write_text(domain.read_text().replace(valid, unsafe))
    entry = _entry(
        scheduled=False,
        parallel_refresh=False,
        orchestration_mode="explicit_targets",
    ...
    assert any(message in item for item in module.check_source("foo", [entry]).missing)
...
def test_registry_rejects_invalid_profile_mode_and_raw_inventory() -> None:
    module = _load_module()
    errors = module.registry_errors(
        [
            _entry(profile="database"),
            _entry("invalid_mode", orchestration_mode="automatic"),
            _entry("explicit_scheduled", orchestration_mode="explicit_targets"),
            _entry(
                "explicit_parallel",
                scheduled=False,
                orchestration_mode="explicit_targets",
    ...
    assert any("invalid orchestration mode" in item for item in errors)
    assert any("explicit-target source must not be scheduled" in item for item in errors)
    assert any("must not participate in parallel refresh" in item for item in errors)
