...
Usage:
    ...
    uv run python scripts/platform/generate_docs.py --check
"""
...
def render_index_page(ctx: Context, contracts: dict[str, SodaContract]) -> str:
    ...
    lines.append(
        ...
        "Regenerate with `uv run python scripts/platform/generate_docs.py`."
    )
    lines.append("")
...
def run_check() -> int:
    ...
    print("docs/dictionary/ is stale. Run `uv run python scripts/platform/generate_docs.py`.\n")
    if missing:
