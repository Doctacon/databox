...
SENSITIVE_PATTERNS = [
    ...
    (r'(?:export\s+)?([A-Z_]+(?:KEY|TOKEN|SECRET|PASSWORD|PASS|PWD|AUTH|CREDENTIAL|API|PRIVATE)(?:[A-Z_]*)?)\s*=\s*["\']?([^"\'\s]+)', 'Environment variable with sensitive name'),
    
    # Direct API keys/tokens
    (r'(?:api[_-]?key|token|secret|password|auth[_-]?token)\s*[:=]\s*["\']([a-zA-Z0-9\-_]{20,})["\']', 'Hardcoded API key/token'),
    ...
    (r'(?:postgresql|postgres|mysql|mongodb|redis)://[^:]+:[^@]+@[^/]+', 'Database URL with credentials'),
...
EXCLUDE_PATTERNS = [
    '.git', '.dlt', '.sqlmesh', '__pycache__', 'node_modules', '.venv', 'venv',
    '*.pyc', '*.pyo', '*.pyd', '.DS_Store', '*.log',
    '.env', '.env.*', 'config/settings.py',  # These are expected to have env vars
]
