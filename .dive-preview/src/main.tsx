...
function ConnectionGate({ children }: { children: React.ReactNode }) {
  ...
  if (isConnecting) return (
    <div className="flex items-center justify-center h-screen gap-2" style={{color:"#6a6a6a"}}>
      <Loader2 className="animate-spin" size={20} />
      Connecting to MotherDuck…
    </div>
  );
  if (error) return (
    <div className="flex items-center justify-center h-screen gap-2" style={{color:"#bc1200"}}>
      <AlertCircle size={20} />
