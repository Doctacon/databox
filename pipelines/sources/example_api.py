...
def example_api_source(api_base_url: str = "https://jsonplaceholder.typicode.com"):
    ...
    @dlt.resource(
        primary_key="id", write_disposition="merge", columns={"id": {"data_type": "bigint"}}
    )
    ...
    @dlt.resource(
        ...
        columns={"id": {"data_type": "bigint"}, "userId": {"data_type": "bigint"}},
    )
    ...
    @dlt.resource(
        primary_key="id",
        write_disposition="merge",
        columns={"id": {"data_type": "bigint"}, "postId": {"data_type": "bigint"}},
    )
...
def load_example_api_data():
    ...
    with pipeline.sql_client() as client:
        ...
        for table in tables:
            ...
            # Show row count
            count = client.execute_sql(f"SELECT COUNT(*) FROM raw_api_data.{table[0]}")
