...
`python scripts/analytics/generate_staging.py --check` remains in CI as a harmless drift
...
`source_table` key and run `python scripts/analytics/generate_staging.py`. Prefer direct
