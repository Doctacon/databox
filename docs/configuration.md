...
| AWS session token | `DATABOX_AWS_SESSION_TOKEN` | `settings.aws_session_token` | Empty for the accepted local long-lived runtime key; required and forwarded when using temporary OIDC/STS credentials, including pgBackRest |
...
For the local primary Iceberg warehouse, Databox accepts the Git-ignored `.env`
access key and secret for `databox-lake-user`; this credential shape has no
session token. `.env` must remain mode `0600` on the FileVault-protected host.
The same bounded runtime identity protects PostgreSQL through pgBackRest's
separate S3 repository. CI likewise reuses its temporary OIDC warehouse-role
credential, including its session token. The MFA-protected catalog-backup role
remains a human recovery identity and is never injected into routine services.
