...
- **Dive preview** (`.dive-preview/`, `scripts/render_cost_page.py`) — generates a cost chart. Active. Fine. But: is it duplicating what Dagster's built-in asset metadata already renders?
- **`scripts/render_cost_page.py`** — regenerates `docs/images/cost/*.png` from `analytics.mart_cost_summary`. Fine, low cost. Keep.
