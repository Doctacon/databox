...
from pipelines.sources.ebird_api import load_ebird_data, load_multiple_regions
...
def list():
    ...
    table.add_row("ebird_api", "Fetch bird observation data from eBird API", "API")
...
@click.option("--region", "-r", help="Region code for eBird pipeline (e.g., US-CA, US-NY)")
@click.option("--days", type=int, default=7, help="Days back for eBird data")
@click.option("--max-results", type=int, default=100, help="Max results per API call")
@click.option("--regions", help="Comma-separated list of regions for eBird")
def run(name: str, file: str = None, directory: str = None, region: str = None, 
        days: int = 7, max_results: int = 100, regions: str = None):
    ...
    try:
        if name == "example_api":
            ...
            if regions:
                ...
                console.print(f"Loading data for regions: {region_list}")
                load_multiple_regions(region_list, max_results=max_results, days_back=days)
            else:
                # Single region (default to US-CA if not specified)
                region_code = region or "US-CA"
                console.print(f"Loading data for region: {region_code}")
                load_ebird_data(
...
@cli.command()
@click.argument("regions", required=False)
def ebird_demo(regions: str = None):
    """Run a demo of the eBird pipeline with sample regions."""
    console.print("[bold cyan]🐦 Running eBird API Demo[/bold cyan]")
    
    if regions:
        region_list = [r.strip() for r in regions.split(",")]
    else:
        # Demo regions
        region_list = ["US-CA", "US-NY", "GB"]
        console.print("Using demo regions: California, New York, Great Britain")
    
    console.print(f"Fetching bird observation data for: {region_list}")
    console.print("Note: Requires EBIRD_API_TOKEN in .env file")
    
    try:
        load_multiple_regions(region_list, max_results=50, days_back=3)
        console.print("[bold green]🎉 eBird demo completed successfully![/bold green]")
    except Exception as e:
        console.print(f"[bold red]Demo failed:[/bold red] {str(e)}")
        console.print("Make sure EBIRD_API_TOKEN is set in your .env file")
        sys.exit(1)
